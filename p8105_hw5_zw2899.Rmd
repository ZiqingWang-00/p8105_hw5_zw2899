...
Below is a plot showing the average estimate of $\hat \mu$ on the y axis and the true value of μ on the x axis.
```{r avg mu hat vs mu}
sim_results %>% 
  group_by(mu) %>%
...
But when the effect size is large ($\mu \leq 4$), the sample average of $\hat \mu$ across tests for which the null was rejected does approximately equal to the true mean.  
